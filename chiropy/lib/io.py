...
class GinpIO:
    def __init__(self, coords, args):
        ...
        self.output = (
            ...
            else f"{self.basename}{self.suffix}.{args.ext}"
        )
...
class GoutIO:
    ...
    def get_coords(self):
        ...
        # capture coordinate
        match_lns = [
            ln for ln, line in enumerate(rl) if start_flag_ptn.match(line) is not None
        ]
    ...
    def get_geometry(self):
        ...
        coords_str = f"{len(coords)}\ntitle\n" + "\n".join(
            [f"{s}    {x}    {y}    {z}" for _, s, x, y, z in coords]
        )
